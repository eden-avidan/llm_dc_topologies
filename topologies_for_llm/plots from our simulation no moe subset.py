# -*- coding: utf-8 -*-
"""
Generate plots from the non-MOE subset (matching MOE configurations)
This creates comparable plots to the MOE analysis.
"""
...
if not os.path.exists(PKL_DIR):
    ...
    print("   Make sure to run 'Topologies Runtime No MOE Subset.py' first")
    exit(1)
...
for filename in PKL_FILES:
    ...
    plt.figure(dpi=500)
    plt.plot(total_runtime_df["file"], total_runtime_df["fat tree"], "o", label="Fat Tree", color="#1f77b4", ms=6)
    plt.plot(total_runtime_df["file"], total_runtime_df["DragonFly+"], "o", label="DragonFly+", color="#ff7f0e", ms=2.5)
    plt.plot(total_runtime_df["file"], total_runtime_df["HyperX"], "o", label="HyperX", color="#2ca02c", ms=3)

    plt.xlabel("Workload index")
    plt.ylabel("Overhead Communication")
    title = f"Overhead Communication of Topologies - {name} (Densed)"
    plt.title(title)
    plt.legend()
    ...
    for size, sub_df in large_group_dfs.items():
        ...
        title = f"Topology Performance Comparison ({name}):\nTraffic Matrices Count per Overhead Communication Range for a Network of {size} GPUs (Densed)"
